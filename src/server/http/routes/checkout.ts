...
import { checkoutData, getOrder, publicOrder, selectOrderPayment, selectOrderPaymentByNetwork, submitPaymentReview, submitTxCandidates } from "@/server/services/orders/service";
...
export function createCheckoutRoutes() {
  ...
  app.post("/:id/payment", async (c) => {
    const body = (await c.req.json()) as { currency?: string; network?: string; payway?: number };
    if (body.network) {
      return c.json(await selectOrderPaymentByNetwork(c.env, c.req.param("id"), String(body.currency ?? ""), String(body.network)));
    }
  ...
  app.post("/:id/review", async (c) => c.json(await submitPaymentReview(c.env, c.req.param("id"), (await c.req.json()) as Record<string, unknown>)));
  return app;
}
