...
func (t *TronAPI) ChainType() Chain {
	return ChainTRON
}
...
type tronDriver struct{}

func (d tronDriver) Meta() Meta {
	return Meta{
		ID:          "chain/tron",
		Name:        "TRON",
		Kind:        "chain",
		Networks:    []string{"tron"},
		Currencies:  []string{"USDT", "TRX"},
		HasQRCode:   true,
		CanScan:     true,
		Description: "TRON 链地址收款",
	}
}

func (d tronDriver) FormSchema() []Field {
	return []Field{
		{Key: "name", Label: "显示名称", Type: "text", Required: true, Placeholder: "TRON 主地址"},
		{Key: "address", Label: "收款地址", Type: "text", Required: true},
		{Key: "currencies", Label: "支持币种", Type: "text", Required: true, Placeholder: "USDT,TRX"},
		{Key: "api_key", Label: "TronGrid API Key", Type: "text"},
		{Key: "confirm_tolerance", Label: "金额容差", Type: "number", Placeholder: "0.000001"},
	}
}

func (d tronDriver) Quote(req QuoteRequest, fx Converter) ([]Quote, error) {
	currencies := csvList(firstNonEmpty(req.Method.Fields["currencies"], "USDT,TRX"))
...
func (d tronDriver) Assign(req AssignRequest, fx Converter) (*Route, error) {
	...
	return &Route{
		...
		Address:      address,
		QRValue:      address,
		Instructions: "请通过 TRON 网络付款，并确保金额精确一致。",
	}, nil
}

func (d tronDriver) Scanner(method Method, debug bool) Scanner {
	return chainScanner{
		api:       NewTronAPI(tronURL(debug), strings.TrimSpace(method.Fields["api_key"])),
		chain:     ChainTRON,
