...
func New(store *store.Store, registry *payments.Registry, cfg *config.Config) *App {
	return &App{Store: store, Registry: registry, Config: cfg}
}

func (a *App) BaseCurrency() string {
	return strings.ToUpper(a.Store.GetConfig("currency", "CNY"))
}
...
func (a *App) adjustment() float64 {
	value := a.Store.GetConfig("rate_adjust", "0")
...
func (a *App) Dashboard() (*Dashboard, error) {
	orders, err := a.Store.ListOrders(8, "all")
	if err != nil {
		return nil, err
	}
	...
	for _, item := range orders {
		if item.CreatedAt.YearDay() == now.YearDay() && item.CreatedAt.Year() == now.Year() && item.Status == store.OrderPaid {
			out.TodayAmount += item.FiatAmount
			out.TodayCount++
		}
		if item.Status == store.OrderPending {
			out.PendingCount++
		}
		if strings.EqualFold(item.NotifyStatus, store.NotifyFailed) {
			out.FailedNotifyCount++
		}
	}
	methods, err := a.Store.ListPaymentMethods()
	if err != nil {
		return nil, err
	}
...
type Checkout struct {
	...
	Merchant *store.Merchant     `json:"merchant"`
	Routes   map[string][]Option `json:"routes"`
	Selected *store.PaymentRoute `json:"selected"`
...
func (a *App) SelectRoute(orderID string, methodID int64, currency string) (*store.PaymentRoute, error) {
	order, err := a.Store.GetOrder(orderID)
	if err != nil {
		return nil, err
	}
	if order.Status != store.OrderPending || time.Now().After(order.ExpireAt) {
		return nil, fmt.Errorf("订单已失效")
	}
	method, err := a.Store.GetPaymentMethod(methodID)
	if err != nil {
		return nil, err
	}
	...
	if err := a.Store.SaveRoute(item); err != nil {
		return nil, err
	}
	return item, nil
}
...
func (a *App) CreateMerchantOrder(apiKey string, req MerchantOrderRequest) (*store.Order, bool, error) {
	merchant, err := a.Store.GetMerchantByAPIKey(apiKey)
	if err != nil {
	...
	if !gateway.Verify(payload, merchant.SecretKey, req.Sign) {
		return nil, false, fmt.Errorf("签名校验失败")
	}
	existing, err := a.Store.GetOrderByMerchantRef(merchant.ID, req.MerchantOrderNo)
	...
	if raw := a.Store.GetConfig("timeout", "1800"); raw != "" {
	...
	if err := a.Store.CreateOrder(item); err != nil {
		return nil, false, err
	}
	return item, false, nil
}

func (a *App) Orders(status string, limit int) ([]store.Order, error) {
	return a.Store.ListOrders(limit, status)
}

func (a *App) Order(orderID string) (*store.Order, error) {
	return a.Store.GetOrder(orderID)
}

func (a *App) Methods() ([]store.PaymentMethod, error) {
	return a.Store.ListPaymentMethods()
}

func (a *App) SaveMethod(item *store.PaymentMethod) error {
	...
	return a.Store.SavePaymentMethod(item)
}
...
func (a *App) SaveSettings(values map[string]string) error {
	return a.Store.SetConfigs(values)
}

func (a *App) SaveMerchant(item *store.Merchant) error {
	if strings.TrimSpace(item.Name) == "" {
		return fmt.Errorf("商户名称不能为空")
	}
	return a.Store.SaveMerchant(item)
}

func (a *App) DeleteMethod(id int64) error {
	return a.Store.DeletePaymentMethod(id)
}

func (a *App) DeleteMerchant(id string) error {
	return a.Store.DeleteMerchant(id)
}

func (a *App) CheckoutStatus(orderID string) (*store.Order, error) {
	order, err := a.Store.GetOrder(orderID)
	if err != nil {
		return nil, err
	}
	if order.Status == store.OrderPending && time.Now().After(order.ExpireAt) {
		_ = a.Store.MarkOrderExpired(order.ID)
		order.Status = store.OrderExpired
	}
	return order, nil
}

func (a *App) InlineOrder(amount float64, currency string, customerRef string) (*store.Order, error) {
	merchants, err := a.Store.ListMerchants()
	if err != nil {
		return nil, err
	}
	merchantID := "INLINE"
...
func (a *App) PostCallback(task store.NotificationTask) error {
	order, err := a.Store.GetOrder(task.OrderID)
	if err != nil {
		return err
	}
	if strings.TrimSpace(order.CallbackURL) == "" {
		return a.Store.MarkNotificationDone(task.ID)
	}
